...
class WeatherCityTest extends TestCase
{
    ...
    public function test_weather_city(): void
    {
        ...
        $city_longitude_latitude = Http::retry(3, 60000)->timeout(500)
    ->connectTimeout(500)->get("https://geocoding-api.open-meteo.com/v1/search?name={$city}&count=1&language=en&format=json")->json()['results'][0];

        $city_weather = Http::retry(3, 60000)->timeout(500)
    ->connectTimeout(500)->get("https://api.open-meteo.com/v1/forecast?latitude={$city_longitude_latitude['latitude']}&longitude={$city_longitude_latitude['longitude']}&current=temperature_2m,precipitation,weather_code,wind_speed_10m,relative_humidity_2m,cloud_cover&timezone=auto&start_date={$current_date}&end_date={$current_date}")->json();
